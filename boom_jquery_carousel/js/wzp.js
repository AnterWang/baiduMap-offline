...
$(function () {
    ...
    //爆炸
    $("#wzpleft,#wzpright").on("click", function () {
        ...
        oDiv.innerHTML = '';
        oDiv.style.background = 'url(wzpimg/wzp3' + (next() + 1) + '.png) center no-repeat';
        var aData = [];
        var wait = R * C;
        ...
        for (var i = 0; i < R; i++) {
            for (var j = 0, k = 0; j < C; j++, k++) {
                ...
                oNewDiv.css({
                    ...
                    background: 'url(wzpimg/wzp3' + (now + 1) + '.png) ' + -aData[i].left + 'px ' + -aData[i].top + 'px no-repeat',
    ...
    //自动播放
    var wzpright = document.getElementById("wzpright");
    wzptimer = setInterval(function () {
        wzpright.click();
    }, 2500);
    $(function () {
        $(".wzp").mouseenter(function () {
            $("#wzparr").css("display", "block");
            clearInterval(wzptimer);
        })
    });
    $(function () {
        $(".wzp").mouseleave(function () {
            $("#wzparr").css("display", "none");
            wzptimer = setInterval(function () {
                wzpright.click();
            }, 2500);
        })
